...
efineday('opendata-ember/adapters/dataset', ['exports', 'ember', 'opendata-ember/adapters/application'], function (exports, Ember, ApplicationAdapter) {
  ...
  exports['default'] = ApplicationAdapter['default'].extend({
    ...
    urlForFindRecord: function urlForFindRecord(id /*, modelName, snapshot*/) {
      Ember['default'].debug('Dataset Adapter: urlForFindRecord called id: ' + id);
    ...
    query: function query(store, type, _query) {
      Ember['default'].debug('>>>>> query');
      _query.filter = { source: 'ESRI R&D Center' };
...
efineday('opendata-ember/controllers/datasets', ['exports', 'ember'], function (exports, Ember) {
  ...
  exports['default'] = Ember['default'].Controller.extend({
    ...
    totalCount: Ember['default'].computed('model.meta.stats.totalCount', function () {
      return this.get('model.meta.stats.totalCount');
    }),

    // The following properties will be used for the display of the pagination links
    totalPages: Ember['default'].computed('totalCount', function () {
      var totalPages = Math.ceil(this.get('totalCount') / this.get('model.meta.queryParameters.page.size'));
      return totalPages;
    }),
    prevPage: Ember['default'].computed('model.meta.queryParameters.page.number', function () {
      return this.get('model.meta.queryParameters.page.number') - 1;
    }),

    nextPage: Ember['default'].computed('model.meta.queryParameters.page.number', function () {
      return this.get('model.meta.queryParameters.page.number') + 1;
    }),

    isFirstPage: Ember['default'].computed.equal('model.meta.queryParameters.page.number', 1),

    isLastPage: Ember['default'].computed('model.meta.queryParameters.page.number', 'totalPages', function () {
      return this.get('model.meta.queryParameters.page.number') >= this.get('totalPages');
    }),

    pageRange: Ember['default'].computed('model.meta.queryParameters.page.number', 'totalPages', function () {
      var result = Ember['default'].A();

      var currentPage = this.get('model.meta.queryParameters.page.number');
      var totalPages = this.get('totalPages');
...
efineday('opendata-ember/models/dataset', ['exports', 'ember-data'], function (exports, DS) {
  ...
  exports['default'] = DS['default'].Model.extend({
    ...
    licenseInfo: DS['default'].attr('string'),
    mainGroupThumbnailUrl: DS['default'].attr('string'),
    ...
    serviceSpatialReference: DS['default'].attr(), //{}
    ...
    site: DS['default'].hasMany('site', { inverse: null, async: true }),
    organization: DS['default'].belongsTo('organization', { inverse: null, async: true }),
    group: DS['default'].hasMany('group', { inverse: null, async: true })
...
efineday('opendata-ember/tests/routes/datasets.jshint', function () {
  ...
  QUnit.test('routes/datasets.js should pass jshint', function(assert) { 
    assert.expect(1);
    assert.ok(false, 'routes/datasets.js should pass jshint.\nroutes/datasets.js: line 29, col 9, \'ctrl\' is defined but never used.\nroutes/datasets.js: line 27, col 28, \'transition\' is defined but never used.\n\n2 errors'); 
  });
...
if (runningTests) {
  ...
  equireray("opendata-ember/app")["default"].create({"API":"https://opendata.arcgis.com","LOG_RESOLVER":true,"LOG_ACTIVE_GENERATION":true,"LOG_TRANSITIONS":true,"LOG_TRANSITIONS_INTERNAL":true,"LOG_VIEW_LOOKUPS":true,"name":"opendata-ember","version":"0.0.0+fa3b6517"});
}
